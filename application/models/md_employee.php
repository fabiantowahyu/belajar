...
class Md_employee extends CI_Model {
    ...
    public function MDL_Select_Sales() {
	$tblemployee = $this->config->item('tmst_employee');
	$tbltypevar = $this->config->item('tmst_typevar');
	$tblbranch = $this->config->item('tmst_branch');
	$tbl_position = $this->config->item('tmst_position');
	...
	$sSQL = "
			...
			FROM $tblemployee a
			LEFT JOIN $tbladdress b ON b.emp_id = a.emp_id
			WHERE 1=1
			AND a.emp_id ='$id'
		";
	...
	$sSQL = "
			SELECT e.id, e.education_lvl, e.startdate, e.enddate, a.nm_type AS faculty, b.nm_type AS institution,c.nm_type AS education_lvl,e.education_lvl as edu_levelid,e.faculty,e.institution,e.country,e.province,e.city,e.major,e.gpa,e.is_default
	...
	if ($ambil->num_rows() > 0) {
		...
		if ($data->family_dependentsts == 1) {
		    $data->family_dependentsts = "Yes";
		} else {
		    $data->family_dependentsts = "No";
		}
		if ($data->alive_status == 1) {
		    $data->alive_status = "Deceased";
		} else {
		    $data->alive_status = "Alive status";
		}
		$hasil[] = $data;
	    }
	}
	...
	$sSQL = "
			SELECT a.*
			FROM $tbl_empbank a, $tbl_employee b
			WHERE 1=1 
	...
	$filepath_avatar = $this->config->item('filepath_avatar');
	$filepath_avatar_tumbs = $this->config->item('filepath_avatar_tumbs');
	$filepath_signature = $this->config->item('filepath_signature');
	$separator = $this->config->item('separator');
	$tblemployee = $this->config->item('tmst_employee');
	...
	//your desired config for the resize() function : Tumbnails
	$config = array(
	    'image_library' => 'gd2',
	    'source_image' => sprintf("%s\%s.%s", $filepath_avatar, $id, $type), //path to the uploaded image
	    'new_image' => sprintf("%s\%s.%s", $filepath_avatar_tumbs, $id, $type),
	    'maintain_ratio' => true,
	    'create_thumb' => true,
	    'width' => 55,
	    'height' => 35
