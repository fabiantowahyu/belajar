...
class Md_menubox extends CI_Model {
	...
	public function MDL_MainMenu_List($name,$arymenu,$step=0,$parram=array()) {
		...
		if(is_array($arymenu) && count($arymenu)) {
			$step += 1;
			$content = '';
			// Remark by Danang for Smart Admin Themes 
			//if($step > 1) {
				$content .= str_repeat("\t",$step) . "<ul>\n";
			//}
			$i=0;
			//$this->auth->TVD($arymenu); exit();
			while(list($k,$v) = @each($arymenu)) {
				$title = $v['title'];
				...
				if(isset($v['sub'])) {
					...
					$content .= "
						<a href=\"".$url_menu."\" >
							<i class=\"".$icon."\"></i> <span class=\"menu-item-parent\"> 
							" . $title . " </span>
						</a>";
				}
				$content .= "</li>\n";
	...
	public function MDL_getAvatar_User(){
		...
		if(strlen($file_name)) {
			...
			$filename = sprintf("file_upload/avatar/thumbnails/%s.%s",$id,$type);
		} else {
			$filename = sprintf("file_upload/avatar/thumbnails/no_photo.jpg");
		}
		
        return $filename;
    }
    public function MDL_getPicture_User(){
		$tblemployee = $this->config->item('tmst_employee');
	...
	public function MDL_getAvatar_User_By_Id($id){
		$tblemployee = $this->config->item('tmst_employee');
