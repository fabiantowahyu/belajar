...
class Manage_user extends CI_Controller {
	...
	public function __construct() {
		...
		//$this->load->model('md_data_authorization');
	...
	public function CTRL_Privileges_User($id='') {
		if(!$this->auth->Auth_isPerm()) {
			...
			if ($this->input->post('close')) {
				...
				$this->breadcrumbs->add($parentt, '#', $path_icon);
				$this->breadcrumbs->add($nm_menu, base_url().$url_menu);
				$this->breadcrumbs->add('Privileges', current_url());
				$breadcrum = $this->breadcrumbs->output();
				$data['breadcrum'] = $breadcrum;
				/* end */
    ...
    public function CTRL_Data_Authorization($id='') {
		if(!$this->auth->Auth_isPerm()) {
			$this->load->view('error_akses');
		} elseif(!$this->auth->Auth_isPrivButton('edit')) {
			$data['action'] = 'privileges';
			...
			if ($this->input->post('close')) {
				...
				//$id = $this->input->post('user_id');
				$this->md_manage_user->MDL_InsertPriv_User();
				redirect('manage_user/CTRL_Data_Authorization/'.$id);
			} else {
				/* Bread crum */
				$this->load->model('md_ref_json');
				$params = $this->router->fetch_class();
				...
				$AryNotMember = $this->CTRL_Option_ActiveData($data['userid']);
				$AryMember = $this->CTRL_Option_ActiveDataUser($data['userid']);
				$data['option_not_member'] = $AryNotMember;
				$data['option_member'] = $AryMember;
				
				$nm_title = $this->auth->Auth_getNameMenu();
				$data['title'] = sprintf("%s - Admin Authorization Group",$nm_title);
				$data['url'] = 'manage_user/CTRL_Authorize/'.$hasil->userid;
				$data['page'] = 'ManageUser/data_authorization';
				$data['plugin'] = 'ManageUser/plugin';
				$this->load->view('template_admin', $data);
			}
		}
    ...
    public function CTRL_Authorize(){
    	$all_data = $this->input->post('all_data');
    	$verified_access = $this->input->post('verified_access');
    	$add = $this->input->post('add');
    	$remove = $this->input->post('remove');
    	$userid = $this->input->post('user_id');
    	if($add){
			$this->md_data_authorization->MDL_Add_Authorization($all_data, $userid);
			print "<script type=\"text/javascript\">alert('This data successfully authorized');
			window.location.href = document.referrer;</script>";
    	}elseif($remove){
    		$this->md_data_authorization->MDL_Remove_Authorization($verified_access);
    		print "<script type=\"text/javascript\">alert('This data has been unauthorized');
				window.location.href = document.referrer;</script>";
    	}
    }

    public function CTRL_Option_ActiveData($userid) {

		$option = array();
		$AryData = $this->md_data_authorization->MDL_Select_Active($userid);
		//$option[''] = 'Choose a Company...';
		foreach($AryData as $row) {
			$option[$row->id] = $row->custom_title;
		}

		return $option;
	}

	public function CTRL_Option_ActiveDataUser($userid) {

		$option = array();
		$AryData = $this->md_data_authorization->MDL_Select_ActiveData($userid);
		//$option[''] = 'Choose a Company...';
		foreach($AryData as $row) {
			$option[$row->id] = $row->custom_title;
		}
