	...
	public function CTRL_Privileges_User($id='') {
		if(!$this->auth->Auth_isPerm()) {
			...
			if ($this->input->post('close')) {
				...
				$this->breadcrumbs->add($parentt, '#', $path_icon);
				$this->breadcrumbs->add($nm_menu, base_url().$url_menu);
				$this->breadcrumbs->add('Admin Authorization Group', current_url());
				//$this->breadcrumbs->add('Spring Tutorial', base_url().'tutorials/spring-tutorials');
				$breadcrum = $this->breadcrumbs->output();
				$data['breadcrum'] = $breadcrum;
				/* end */
	...
	public function CTRL_Privileges_Menu($id='') {
		if(!$this->auth->Auth_isPerm()) {
			...
			if ($this->input->post('close')) {
				...
				redirect('manage_group/CTRL_Edit/'.$id);
			} else {
				/* Bread crum */
				$this->load->model('md_ref_json');
				$params = $this->router->fetch_class();
				...
				$this->breadcrumbs->add($parentt, '#', $path_icon);
				$this->breadcrumbs->add($nm_menu, base_url().$url_menu);
				$this->breadcrumbs->add('User Authorization Group', current_url());
				$breadcrum = $this->breadcrumbs->output();
				$data['breadcrum'] = $breadcrum;
				/* end */
					...
					if($step > 1) {
						...
						$content .= '
							<div class="row-fluid">
								<div class="col-sm-7">
									<label>
										<input name="' . $name . '" class="checkbox style-0" type="checkbox" value="' . $id . '" ' . $checked . '>
										<span class="lbl"> ' . $title .'</span>
									</label>
								</div>
								<div class="span5">
									<span>Access : </span>
									<label>
										<input name="' . $name_access . '" type="radio" class="radiobox style-0" value="list" ' . $checked_list . '>
										<span class="lbl"> List</span>
									</label>
									<label>
										<input name="' . $name_access . '" type="radio" class="radiobox style-0" value="add" ' . $checked_add . '>
										<span class="lbl"> Add</span>
									</label>
									<label>
										<input name="' . $name_access . '" type="radio" class="radiobox style-0" value="edit" ' . $checked_edit . '>
										<span class="lbl"> Edit</span>
									</label>
									<label>
										<input name="' . $name_access . '" type="radio" class="radiobox style-0" value="delete" ' . $checked_delete . '>
										<span class="lbl"> Delete</span>
									</label>
								</div>
							</div>
