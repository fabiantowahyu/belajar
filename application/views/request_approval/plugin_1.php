...
<script src="<?php echo base_url();?>themes/js/chosen.jquery.min.js"></script>
...
<script type="text/javascript">
	$(function() {
		var oTable1 = $('#table-approval').dataTable( {
		"aoColumns": [
		  { "bSortable": true },
		  null,null,
		  { "bSortable": false }
		...
		$("#cdelete").on(ace.click_event, function() {
			bootbox.dialog({
				message: "Are you sure, you want to delete this group menu!",
	...
	function cekKey(event,objSource,objTarget)
	{
		...
		if (event.keyCode == 13)
		{
			...
			if (document.myForm.search_not_member.value != "")
			{
				decoder('0',objSource,objTarget,'false');
				event.returnValue = false;
				return false;
			}
		}
	...
	function decoder(start,objSource,objTarget,flag){
		var tmpID=" "
		var preciseFlag = false;
		var searchKey = document.myForm.search_not_member.value;
		if (preciseFlag == false) {
			for (ii=parseInt(start);ii<objSource.options.length;ii++) {		
				if (objSource.options[ii].text.toUpperCase().indexOf(searchKey.toUpperCase()) >= 0){
					if (flag == 'true') {
						notSelected(objSource);
					}
					objSource.options[ii].selected = true;
					objSource.focus();
					tmpID=tmpID+","+ii
					break;
				}
		...
		volt = tmpID.split(",").length - 1
		if (volt == 0){
			alert("<cfoutput>Data is not found</cfoutput>");
		}
